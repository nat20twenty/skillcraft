...
public class SkilltreePickerWidget extends AbstractWidget {
    ...
    protected void renderWidget(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        int y_offset = 0;
        Component name = Component.translatable(this.tree.getTreeName());

        //If any level of the perk is unlocked, use the clicked icon.
        float v = 0.0f;
        if (SkillCraftPerkScreen.getSelectedTree().equals(this.tree)) {
            v = 24.0f;
            y_offset = height/16;
        }
    ...
    public void onClick(MouseButtonEvent click, boolean doubled) {
        SkillCraftPerkScreen.setSelectedPerk(null);
        SkillCraftPerkScreen.setSelected(this.tree);
        SkillCraftPerkScreen.setOffset_x(0d);
        SkillCraftPerkScreen.setOffset_y(0d);
        SkillCraftPerkScreen.setOffset_bounds(this.tree.getOffset_bounds());
    }
}
